'''a module to define a cache class for files
'''
...
class FileCache(Cache.Cache):
    '''a class to maintain a cache of an user files
    '''
    ...
    def insert_url(self, url, retrieve=None):
        ...
        path = os.path.join(tempfile.gettempdir(), "files")
        try:
            retrieve(url, path)
        except IOError:
