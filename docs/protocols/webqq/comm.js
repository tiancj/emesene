...
var pt = {
    ...
    go_qrlogin_step: function (A) {
        ...
        case 2:
            ...
            break;
        case 3:
            sys.$("qr_nick").innerHTML = pt.qr_nick;
...
function ptuiCB(J, K, B, H, C, A) {
    function F() {
        var N = pt.cookie.get("uin");
        var L = pt.cookie.get("skey");
        if (N == "" || L == "") {
            var M = new Image();
            M.src = "http://id.qq.com/cgi-bin/infolog?br=" + browser_version() + "&r=" + Math.random() + "&u=" + $("u").value
        }
    }
    function D() {
        if (H != "0") {
    ...
    if (J == 67) {
        ...
        window.clearInterval(pt.qrlogin_clock);
        pt.qrlogin_clock = pt.qrlogin_clock = window.setInterval("qrlogin_submit();", 1000);
        return
    }
    if (J == 0) {
        var G = 0;
